...
freq=np.fft.fftfreq(len(trans),dt)
plt.figure()
filtrada=np.where(abs(freq)<=1000,trans,0)
ampNueva=np.fft.ifft(filtrada)
# SU GRAFICA

plt.figure()
plt.plot(t,amp,color="black")
plt.plot(t,ampNueva,color="red")
